...
public class ItemTransmitterNodeMenu extends AbstractContainerMenu {
    public final ItemTransmitterNodeBlockEntity blockEntity;
    private final Level level;
    private final ContainerLevelAccess access;

    public ItemTransmitterNodeMenu(int containerId, Inventory inv, FriendlyByteBuf extraData) {
        this(containerId, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()));
    }

    public ItemTransmitterNodeMenu(int containerId, Inventory inv, BlockEntity entity) {
        super(JTMenuTypes.ITEM_TRANSMITTER_NODE_MENU.get(), containerId);
        checkContainerSize(inv, 2);
        blockEntity = ((ItemTransmitterNodeBlockEntity) entity);
