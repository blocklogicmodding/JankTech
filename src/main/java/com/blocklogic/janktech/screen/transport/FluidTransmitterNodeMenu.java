...
public class FluidTransmitterNodeMenu extends AbstractContainerMenu {
    public final FluidTransmitterNodeBlockEntity blockEntity;
    private final Level level;
    private final ContainerLevelAccess access;

    public FluidTransmitterNodeMenu(int containerId, Inventory inv, FriendlyByteBuf extraData) {
        this(containerId, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()));
    }

    public FluidTransmitterNodeMenu(int containerId, Inventory inv, BlockEntity entity) {
        super(JTMenuTypes.FLUID_TRANSMITTER_NODE_MENU.get(), containerId);
        checkContainerSize(inv, 2);
        blockEntity = ((FluidTransmitterNodeBlockEntity) entity);
